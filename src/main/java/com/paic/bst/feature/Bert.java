...
public class Bert implements AutoCloseable{
    ...
    public static Bert load(Path path) {
        ...
        ModelDetails model;
        try {
            model = new ObjectMapper().readValue(path.resolve(MODEL_DETAILS).toFile(), ModelDetails.class);
        } catch(final IOException e) {
            throw new RuntimeException(e);
        }

        return new Bert(SavedModelBundle.load(path.toString(), "serve"), model, path.resolve(VOCAB_FILE));
    }

//    public static Bert load(String path) {
//        ModelDetails model;
//        try {
...
//        }
    ...
    public float[][] embedTokens(final String sequence) {
        try(Inputs inputs = getInputs(sequence)) {
            ...
            try(Tensor<?> embedding = output.get(0)) {
                final float[][][] converted = new float[1][(int)embedding.shape()[0]][(int)embedding.shape()[1]];embedding.copyTo(converted);
                return converted[0];
            }
        }
    }
